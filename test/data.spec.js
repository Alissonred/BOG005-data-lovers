...
describe('computeStatus', () => {
  ...
  it('Returns percentage of characters', () => {
    let computexpected = (original_dataCompute.results[1].episode.length / 31) * 100;
    expect(computeStatus(original_dataCompute.results[1])).toEqual(computexpected);
    expect(typeof computeStatus(original_dataCompute.results[1])).toBe('number');
  });

  it('computeStatus returns a TypeError', () => {                      // testea el error 
    expect(() => computeStatus()).toThrow(TypeError);
    expect(() => computeStatus(null)).toThrow(TypeError);
    expect(() => computeStatus([])).toThrow(TypeError);
  });
